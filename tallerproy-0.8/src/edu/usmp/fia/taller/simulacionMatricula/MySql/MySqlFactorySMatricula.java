...
public class MySqlFactorySMatricula implements DAOFactorySMatricula {
	...
	@Override
	public List<Area> SimulacionIncial(int condition) throws Exception {
	...
	public List<Area> SimulacionConcluyente(int condicion) throws Exception {
		...
		try {
			...
			for (String alumnoId : alumnoIdList) {
				...
				System.out.println("EJECUTO SCRIPT");
				while (rs.next()) {
...
//				System.out.println("----------->"+rs.getString("seccion"));
	...
	public boolean EliminarPreMatricula(int correlativo) throws Exception {
		...
		try 
		{
			...
			con.rollback();
			System.out.print(e.getMessage());
			throw new Exception("Error al eliminar la pre-matricula detalle");			
			//return null;
		}
		finally
		{
	...
	private boolean LimpiarConexion(Connection con, StringBuffer sql,PreparedStatement ps, ResultSet rs) throws Exception
	{
		try {
